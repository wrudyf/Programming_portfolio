...
let test_is_even_odds = 
  Test.make
    ~name:"test_is_even_odds"
    ~count:10
    (int_range (-1000000) (1000000))
    (fun x -> not(is_even (2*x+1)))

(* All numbers of the form 2x are even *)
let test_is_even_evens =
  Test.make
    ~name:"test_is_even_evens"
    ~count:10
    (int_range (-1000000) (1000000))
    (fun x -> is_even (2*x))
...
(* Product of two numbers is only even if one
   operand is even *)
let test_is_even_multiplicativity = 
  Test.make
    ~name:"test_is_even_multiplicativity"
    ~count:10
    (pair small_int small_int)
    (fun (x, y) -> if is_even (x) || is_even (y) then is_even(x * y) else not(is_even (x * y)))
...
let test_is_odd_additivity =
  ...
  ~count:10
  (pair small_int small_int)
  (fun (x, y) -> if is_even (x) <> is_even (y) then not (is_even (x + y)) else is_even (x + y))
...
let suite = 
  "public" >::: [
    ...
    QCheck_runner.to_ounit2_test test_is_even_odds; 
    QCheck_runner.to_ounit2_test test_is_even_evens;
    QCheck_runner.to_ounit2_test test_is_odd_additivity;
    QCheck_runner.to_ounit2_test test_is_even_multiplicativity;
