...
(define (test-runner run)
  ...
  (check-equal? (run '(and #t)) #t)
  (check-equal? (run '(and #f)) #f)
  (check-equal? (run '(and 8)) 8)
  (check-equal? (run '(and #f 8)) #f)
  (check-equal? (run '(and 1 9 8)) 8)
  (check-equal? (run '(and (and #t) (and #t))) #t)
  (check-equal? (run '(and (and #t) (and #f))) #f)
...
(define (test-runner-io run)
  ;; And examples
  (check-equal? (run '(and 9 (write-byte 97)) "") (cons (void) "a"))
  (check-equal? (run '(and (write-byte 97) #f (write-byte 98)) "") (cons #f "a"))
