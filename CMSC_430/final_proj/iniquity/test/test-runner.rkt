...
;; Abscond examples
  ...
  (check-equal? (run '(define (f x y) y)
   ...
   ;;(check-equal? (run '(values 1 2 3 4) ) 9)
   ;;"arity" checking, ok
   (check-equal? (run '(let-values ([(x y) (values  2)]) (+ x y)) ) 'err)
   ...
   (check-equal? (run '(let-values ([(x) (values 11)]) (add1 x)) ) 12)

   ;;i don't know what to do here
   (check-equal? (run '(add1 (values 4)) ) 5)
   ...
   (check-equal? (run '(let ([x (values 5)]) (add1 x)) ) 6)

   (check-equal?* '[(values 1 2 3)] '(1 2 3))

   (check-equal?* '[(values 1 2 #f)] '(1 2 #f))
   (check-equal?* '[(values 1 2 3 4)] '(1 2 3 4))
   (check-equal? (run '(+ (values 3) (values 4 ))) 7)
   (check-equal? (run '(values ) '()))
)
