...
(define (compile-op0 p)
  (match p
    ...
    ['peek-byte (seq pad-stack
                     (Call 'peek_byte)
                     unpad-stack)]))
;;idea, check to see if what is in rax is a values type first, then check to see if
;;the values has only one arg for check_one that will be called in every op1 primitive
;;if it does have one arg, then move that arg into the rax register
(define (check_one x)
    (let ((l1 (gensym 'a))
          (le (gensym 'b))
          )
