...
(define (test-runner-io run)
  ...
  ;;only test i need to check that my repeat works, and it works flawlessly
  (check-equal? (run '(repeat 3 (write-byte 97)) "") (cons (void) "aaa"))
  (check-equal? (run '(repeat 0 (write-byte 97)) "") (cons 'err ""))
  (check-equal? (run '(repeat #f (write-byte 97)) "") (cons 'err ""))
  )
