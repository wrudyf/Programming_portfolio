...
(define (bigrams s)
  ...
  [0 '()]
  [1 '()]
  [_ (cons (list (substring s 0 1) (substring s 1 2)) (bigrams (substring s 1))) ]))
...
;; Compute the height of a binary tree (leaf has height 0)
 (define (btn-height bt)
  (match bt
  [(leaf) 0]
  [(node n left right) (if (< (btn-height right) (btn-height left)) (+ 1 (btn-height left)) (+ 1 (btn-height right))) ]))
...
(define (btn-contains? bt n)
  (match bt
  [(leaf) #f]
  [ (node n2 left right) (or (if (equal? n2 n) #t (btn-contains? left n)) (if (equal? n2 n) #t (btn-contains? right n)))]))
...
;; HINT: append is a function that might be helpful
  ...
  (match btn
  [(leaf) '()]
  [(node n left right) (cons n (cons (btn-preorder left) (btn-preorder right)))]))
...
(define (expr-lambda-vars e)
  ...
  [(Lam x e) (append (list x) (expr-lambda-vars e))]))
