    ...
    [(DefaultDefn f xs x d e)
     (let ((l1 (gensym 't))
        ...
        (seq
          ...
          ;;if args do not match parameters, jump to l1 label
          (Jne l1)

          ;;if args matches parameters, do this stuff
          (compile-e e (cons x (reverse xs)))
          (Add rsp (* 8 (add1 (length xs)))) ; pop args
          (Ret)

          ;;if args don't match, do this stuff
          (Label l1)
          (compile-e d '()) ;;compile default val to get into rax
          (Push rax)        ;;push val into stack
          (compile-e e (cons x (reverse xs)))
          (Add rsp (* 8 (+ 0 (add1 (length xs)) ) )) ; pop args
          (Ret)
