...
(define (parse-define s)
  (match s
    ;; NOTE: New.
    ;; NOTE: This has to go first because it's a more specific version of the
    ;; same pattern.
    [(list 'define (list (? symbol? f) xs ... (list x d)) e)
     (if (and (andmap symbol? xs)
              (symbol? x))
         (DefaultDefn f xs x (parse-e d) (parse-e e))
         (error "parse default definition error"))]
    ;; NOTE: Updated.
    [(list 'define (list-rest (? symbol? f) xs) e)
     (if (andmap symbol? xs)
         (PlainDefn f xs (parse-e e))
         (error "parse plain definition error"))]
