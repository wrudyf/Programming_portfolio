...
(define (compile-op2 p)
  (match p
    ...
    ;; TODO: Implement this!
    ['set-box!
     (seq (Pop r8)                 ;;pop box into r8 to follow push/pop stack discipline
          (Sub rbx 8)         ;;subtract 8 to change heap pointer so we can change original box
          (Mov (Offset rbx 0) rax) ;;write e2 val in rax into memory
          (Mov rax rbx)            ;;move box into rax
          (Or rax type-box)        ;;tag rax as a box
          (Add rbx 8)              ;;increase heap pointer to point to next avail space in heap
     )]
